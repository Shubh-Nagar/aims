...
**Content blocks.** `src/data/pages.js` entries declare an ordered `blocks` array; each entry names a `type` from the registry in `src/components/blocks/index.jsx` and carries data only. Available types: `prose`, `stats`, `meters`, `table`, `cards`, `chips`, `specs`, `documents`, `gallery`, `figure`, `floors`, `quote`. Adding a page means writing data, never a component — add a new block type only when an existing one genuinely can't carry the shape. Any block with a `heading` becomes an anchor in the sticky `<SectionNav>` scroll-spy rail; pass `navLabel` to shorten its rail label.

Choose the block that carries the *meaning*: `meters` where relative scale is the story (beds per department, built-up area) rather than `table`, which flattens it; `specs` for key/value disclosure rows that may contain links; `cards` where items are genuinely distinct things.

**Migration-pending pattern.** Content pages the original fetch couldn't retrieve (Elementor-rendered body copy) are marked with a `pendingSource` URL in `src/data/pages.js` instead of invented text — `ContentPage.jsx` renders an explicit "content migration pending" notice with a link to the live page. When filling one in, add a `blocks` array and delete `pendingSource`. Never invent placeholder copy for these. As of the last migration pass no page in `pages.js` is pending; the mechanism remains for future additions. The older `sections` shape (heading/body/list/documents) still renders and is used by a few pages.
