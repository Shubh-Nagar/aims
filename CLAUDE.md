...
**Content blocks.** `src/data/pages.js` entries declare an ordered `blocks` array; each entry names a `type` from the registry in `src/components/blocks/index.jsx` and carries data only. Available types: `prose`, `stats`, `meters`, `table`, `cards`, `chips`, `specs`, `documents`, `gallery`, `floors`, `quote`. Adding a page means writing data, never a component — add a new block type only when an existing one genuinely can't carry the shape. Any block with a `heading` becomes an anchor in the sticky `<SectionNav>` scroll-spy rail; pass `navLabel` to shorten its rail label.
